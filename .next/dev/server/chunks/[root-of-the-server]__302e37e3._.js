...
class UserService {
    ...
    static async deleteAccount(userId) {
        ...
        try {
            ...
            try {
                const Pet = __turbopack_context__.r("[project]/models/Pet.ts [app-route] (ecmascript)").default;
                const deletedPets = await Pet.deleteMany({
                    userId: userId
                });
                console.log(`✅ ${deletedPets.deletedCount} animaux supprimés`);
            } catch (error) {
                console.log("ℹ️ Modèle Pet non disponible ou pas d'animaux à supprimer");
            }
            ...
            try {
                ...
                healthData.pets = healthData.pets.filter((pet)=>pet.userId !== userId);
            ...
            try {
                const TrainingSession = __turbopack_context__.r("[project]/models/TrainingSession.ts [app-route] (ecmascript)").default;
                const deletedSessions = await TrainingSession.deleteMany({
                    userId: userId
                });
                console.log(`✅ ${deletedSessions.deletedCount} sessions d'entraînement supprimées`);
            } catch (error) {
            ...
            console.log(`🎉 Suppression complète du compte ${userId} terminée avec succès`);
            return true;
        } catch (error) {
            console.error(`❌ Erreur lors de la suppression du compte ${userId}:`, error.message);
...
async function DELETE(request) {
    try {
        ...
        const deleted = await __TURBOPACK__imported__module__$5b$project$5d2f$services$2f$userService$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["UserService"].deleteAccount(authResult.userId);
        if (!deleted) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__302e37e3._.js.map