...
export async function listPets(): Promise<Array<{ petId: string; name?: string; species?: string }>> {
  const db = await readDB();
  return (db.pets || []).map((p: any) => ({ petId: p.petId, name: p.name, species: p.species }));
}

/**
 * Keep createPet for compatibility but note that pet creation should be centralized.
 */
export async function createPet(name: string, species?: string): Promise<PetHealth> {
...
export async function addRecord(petId: string, payload: Partial<HealthRecord>): Promise<HealthRecord> {
  requirePetId(petId);
  const db = await readDB();
...
export async function addAppointment(petId: string, payload: Partial<MedicalAppointment>): Promise<MedicalAppointment> {
  requirePetId(petId);
  const db = await readDB();
  const idx = (db.pets || []).findIndex((p: any) => p.petId === petId);
  if (idx === -1) throw new HTTPError('Pet not found', 404);
...
export async function addVaccination(petId: string, payload: Partial<Vaccination>): Promise<Vaccination> {
  requirePetId(petId);
  const db = await readDB();
  const idx = (db.pets || []).findIndex((p: any) => p.petId === petId);
  if (idx === -1) throw new HTTPError('Pet not found', 404);
...
export async function deleteRecord(petId: string, recordId: string): Promise<HealthRecord> {
  requirePetId(petId);
  ...
  if (idx === -1) throw new HTTPError('Pet not found', 404);
  db.pets[idx].healthRecords = db.pets[idx].healthRecords || [];
  const recIdx = db.pets[idx].healthRecords.findIndex((r: any) => r.id === recordId);
...
export async function deleteAppointment(petId: string, appointmentId: string): Promise<MedicalAppointment> {
  requirePetId(petId);
  ...
  if (idx === -1) throw new HTTPError('Pet not found', 404);
  db.pets[idx].appointments = db.pets[idx].appointments || [];
  const aIdx = db.pets[idx].appointments.findIndex((a: any) => a.id === appointmentId);
...
export async function deleteVaccination(petId: string, vaccinationId: string): Promise<Vaccination> {
  requirePetId(petId);
  ...
  if (idx === -1) throw new HTTPError('Pet not found', 404);
  db.pets[idx].vaccinations = db.pets[idx].vaccinations || [];
  const vIdx = db.pets[idx].vaccinations.findIndex((v: any) => v.id === vaccinationId);
...
export async function updateVital(petId: string, payload: Partial<VitalStats>): Promise<VitalStats> {
  requirePetId(petId);
  const db = await readDB();
  const idx = (db.pets || []).findIndex((p: any) => p.petId === petId);
  if (idx === -1) throw new HTTPError('Pet not found', 404);
